...
void loop(void) {
  ...
  for (int i = 0; i < squares.size(); i++) {
    for (int j = 0; j < squares[i].size(); j++) {
      ...
      if (squares[i][j] == 0) {
        ...
        canvas.fillRect(i * SQUARE_SIZE, j * SQUARE_SIZE, SQUARE_SIZE,
                        SQUARE_SIZE, color);
      }
      if (squares[i][j] == 1) {
  ...
  for (int i = 0; i < pong.size(); i++) {
    ...
    // 各クラスに応じてボールの色を設定
    if (pong[i].class_num == 0) color = M5.Lcd.color565(200, 100, 81);
    if (pong[i].class_num == 1) color = M5.Lcd.color565(237, 223, 214);
    if (pong[i].class_num == 2) color = M5.Lcd.color565(00, 139, 171);
    if (pong[i].class_num == 3) color = M5.Lcd.color565(103, 193, 173);

    // ボールを描画
    canvas.fillCircle(pong[i].x, pong[i].y, SQUARE_SIZE / 2, color);

    // 四角形との衝突判定と反射、境界との衝突判定
    updateSquareAndBounce(numSquaresX, numSquaresY, pong[i].x, pong[i].y,
                          pong[i].dx, pong[i].dy, pong[i].class_num);
    checkBoundaryCollision(img_width, img_height, pong[i].x, pong[i].y,
                           pong[i].dx, pong[i].dy);
